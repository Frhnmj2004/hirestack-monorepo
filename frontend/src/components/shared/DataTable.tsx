...
export function DataTable<T>({
    ...
    return (
        <div className="flex flex-col gap-4">
                    ...
                    <tbody className="divide-y divide-brand-gray dark:divide-white/5">
                        {data.length === 0 ? (
                            ...
                            data.map((item, rowIndex) => (
                                <tr key={rowIndex} className="hover:bg-brand-violet/5 dark:hover:bg-white/[0.02] transition-colors">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
                <div className="flex items-center justify-between px-2 text-sm text-brand-light-textSecondary dark:text-white/60">
                    <div>
                        Showing <span className="text-brand-light-textPrimary dark:text-white font-medium">{(currentPage - 1) * pageSize + 1}</span> to <span className="text-brand-light-textPrimary dark:text-white font-medium">{Math.min(currentPage * pageSize, totalItems)}</span> of <span className="text-brand-light-textPrimary dark:text-white font-medium">{totalItems}</span> results
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => onPageChange && onPageChange(currentPage - 1)}
                            disabled={currentPage === 1}
                            className="p-1 rounded hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed text-brand-light-textPrimary dark:text-white"
                        >
                            <ChevronLeft className="w-5 h-5" />
                        </button>
                        <span className="px-3 py-1 rounded bg-brand-violet/10 dark:bg-brand-violet/20 text-brand-violet dark:text-brand-lavender font-medium">
                            {currentPage}
                        </span>
                        <button
                            onClick={() => onPageChange && onPageChange(currentPage + 1)}
