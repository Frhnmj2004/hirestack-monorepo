            ...
            <div className="bento-section grid gap-2 p-3 max-w-[54rem] select-none relative" ref={gridRef}>
                <div className="card-responsive grid gap-2">
                    {cardData.map((card, index) => {
                        const baseClass = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-colors duration-300 ease-in-out hover:-translate-y-0.5 ${enableBorderGlow ? 'card--border-glow' : ''}`;
                        const cardStyle = { backgroundColor: card.color || '#060010', borderColor: '#392e4e', color: '#fff' } as React.CSSProperties;
                        return enableStars ? (
                            <ParticleCard key={index} className={baseClass} style={cardStyle} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                                <div className="text-white text-base">{card.label}</div>
                                <div className="text-white">
                                    <h3 className={`font-normal text-base mb-1 ${textAutoHide ? 'line-clamp-1' : ''}`}>{card.title}</h3>
